...
const toDataOrgElement = (data, idx) => Object.assign({}, {
    _section_id: (idx < 2 ? 1 : Math.floor(idx / 3) + 1),
    _entry_id: idx % 3 + 1,
    double_value: String(1 / (3 * (idx + 1)) * idx)
});
const createData = length => new Array(isNaN(length) ? 10 : length).fill({
    _section_id: null,
    _entry_id: null,
    double_value: null
}).map(toDataOrgElement);
